...
Patch numbers advance by exactly one per release: `0.1.2` → `0.1.3` →
`0.1.4`. Never skip or jump patch numbers (`0.1.2` → `0.1.5` is wrong), even
...
gemchain guard ask-computer-mcp
gemchain update ask-computer-mcp <new-version> --dry-run
gemchain update ask-computer-mcp <new-version>
```
...
3. Build passes — `bundle exec rake build` (or
   `gem build ask-computer-mcp.gemspec`).
4. Changelog — `Unreleased` entries moved under the new `X.Y.Z` heading with
   the release date.
5. Version — bumped in `lib/ask/computer/mcp/version.rb` by gemchain, not by
   hand.
...
| `lib/ask/computer/mcp/version.rb` at HEAD | `X.Y.Z` |
| Git tag | `vX.Y.Z` |
...
- Bug fix only: `0.1.2` → `0.1.3`.
- Several bug fixes ship together: one release, `0.1.2` → `0.1.3` (never
  `0.1.5`).
...
- This gem bumped because a dependency released: patch only, e.g. `0.1.3` →
  `0.1.4`, via the gemchain cascade — never hand-published.
